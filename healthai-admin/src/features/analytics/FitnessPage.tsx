...
export default function FitnessPage() {
    ...
    const { data, isLoading, isError, error } = useQuery<import('@/types').AnalyticsPageData, Error>({
        queryKey: ['analytics', 'fitness', range],
        queryFn: () => fetchFitnessData(range),
    });

    if (isLoading) return <LoadingState />;
    if (isError) return <ErrorState message={getErrorMessage(error, 'Erreur lors du chargement des données fitness.')} />;
    if (!data) return null;
